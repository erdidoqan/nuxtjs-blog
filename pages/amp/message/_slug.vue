...
export default {
  ...
  async asyncData({ $content, params, $axios}) {
    ...
    const relateds = await $axios.$get(process.env.API_URL + '/category/message')
    ...
    breadcrumbs() {
      return {
        lists: [
          ...
          { name: this.article.title, url: "/message/" + this.article.slug, ok: false, hidden: true}
        ],
      }
    }
  },
          ...
          "itemListElement": [
            this.relateds.map((related, index) => {
              return {
                "@type": "ListItem",
                "position": index + 1,
                "url": 'https://' + process.env.PUBLISH_URL + '/message/' + related.slug,
                "name": related.title
              }
            })
          ]
  ...
  head() {
    return {
      ...
      link: [
        ...
        { rel: 'canonical', href: 'https://' + process.env.PUBLISH_URL + '/message/' + this.article.slug }
      ],
      script: [{
