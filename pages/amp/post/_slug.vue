<template>
  <div class="">
    ...
    <div class="px-4 mx-auto sm:px-6 xl:max-w-[95%] xl:px-0 mt-10">
      ...
      <ArticlesRelated :category="'post'" :articles="relateds" />
    </div>
  </div>
</template>
<script>
    ...
    breadcrumbs() {
      return {
        lists: [
          ...
          { name: this.article.title, url: "/post/" + this.article.slug, ok: false, hidden: true}
        ],
      }
    }
  },
  ...
  head() {
    return {
      ...
      link: [
        ...
        { rel: 'canonical', href: 'https://' + process.env.PUBLISH_URL + '/post/' + this.article.slug }
      ],
      script: [{
