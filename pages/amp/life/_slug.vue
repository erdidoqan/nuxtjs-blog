<template>
  <div class="">
    ...
    <div class="px-4 mx-auto sm:px-6 xl:max-w-[95%] xl:px-0 mt-10">
      ...
      <ArticlesRelated :category="'life'" :articles="relateds" />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  async asyncData({ $content, params, $axios}) {
    ...
    const relateds = await $axios.$get(process.env.API_URL + '/category/life')
    ...
    breadcrumbs() {
      return {
        lists: [
          ...
          { name: this.article.title, url: "/life/" + this.article.slug, ok: false, hidden: true}
        ],
      }
    }
  },
          ...
          "itemListElement": [
            this.relateds.map((related, index) => {
              return {
                "@type": "ListItem",
                "position": index + 1,
                "url": 'https://' + process.env.PUBLISH_URL + '/life/' + related.slug,
                "name": related.title
              }
            })
          ]
  ...
  head() {
    return {
      ...
      link: [
        ...
        { rel: 'canonical', href: 'https://' + process.env.PUBLISH_URL + '/life/' + this.article.slug }
      ],
      script: [{
