    ...
    <header class="pt-10 pb-5 sm:pb-16 sm:text-center">
      <nav aria-label="breadcrumbs" class="flex items-center justify-center bg-grey-light rounded font-sans w-full">
        <ol itemscope="" itemtype="https://schema.org/BreadcrumbList" class="list-reset flex text-grey-dark">
          ...
          <li itemprop="itemListElement" itemscope="" itemtype="https://schema.org/ListItem">
            <a itemprop="item" href="/message" data-vars-ga-ux-element="Breadcrumbs"
               data-vars-ga-call-to-action="Blogs"
               data-vars-ga-outbound-link="/message"
               class="text-blue-800 font-bold underline">
      ...
      <p class="max-w-3xl mx-auto text-lg text-slate-700 dark:text-slate-400 p-5">
        Welcome to our exceptional world of captivating message posts, carefully curated just for you! We take immense
      ...
      <section class="text-gray-600 body-font">
        <div class="container px-5 py-2 mx-auto">
          <div class="flex flex-wrap -m-4">
            <Articles :category="'message'" :articles="articles" />
          </div>
        </div>
      </section>
    </div>
...
export default {
  async asyncData({ $content, params, $axios }) {
    try {
      const articles = await $axios.$get(process.env.API_URL + '/category/message')
    ...
    jsonld() {
      return {
        ...
        "itemListElement": [
          this.articles.map((article, index) => {
            return {
              "@type": "ListItem",
              "position": index + 1,
              "url": 'https://' + process.env.PUBLISH_URL + '/message/' + article.slug,
              "name": article.title
            }
          })
        ],
        ...
        "mainEntityOfPage": {
          ...
          "@id": 'https://' + process.env.PUBLISH_URL + '/message/',
        ...
        "url": 'https://' + process.env.PUBLISH_URL + '/message/'
      }
    },
  },
  head() {
